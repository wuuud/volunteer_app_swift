...
class EditViewController: UIViewController {
    ...
    func updateRequest(token: String, image: UIImage, volunteerOfferId: Int) {
        ...
        // 要確認                      文字情報と画像やファイルを送信するときは 「AF.upload(multipartFormData: …」 を使う
        AF.upload(
            ...
            case .success:
                /*  ここに更新成功のときの処理  下で定義*/
                self.completionAlart(title: "更新完了!", message: "ボランティア情報を更新しました")
...
//    削除リクエスト(DELETE)
    func deleteRequest(token: String, volunteerOfferId: Int){
        ...
        AF.request(
            ...
            switch response.result {
            case .success:
                self.completionAlart(title: "削除完了", message: "ボランティア情報を削除しました")
            case .failure(let err):
                print(err)
            }
        }
    ...
    func updateAlert(token: String, image: UIImage, volunteerOfferId: Int) {
        let alert = UIAlertController(title: "更新しますか?", message: "この情報を更新してもよろしいですか?", preferredStyle: .alert)
    ...
    func deleteAlert(token: String, volunteerOfferId: Int) {
        let alert = UIAlertController(title: "削除しますか?", message: "この情報を削除します", preferredStyle: .alert)
